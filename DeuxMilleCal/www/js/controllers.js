...
// here functions that are needed globally are defined
    .run(function ($rootScope, homeFactory) {

        $rootScope.formatIntegerToTwoDigits = function (val) {
            return val < 10 ? '0' + val.toString() : val.toString();
        }

        $rootScope.loadUsersCols = function () {
            $rootScope.conquredCols_data = [];
            $rootScope.show();
            var link = 'http://www.deuxmille.cc/api/user/get_usercols/?user_id=' + $rootScope.userID + '&insecure=cool';
            homeFactory
                .colsCall(link, 'GET')
                .then(function (response) {
                    ...
                    for (var col in result) {
                        ...
                        var d = new Date(0);
                        d.setUTCSeconds(parseInt(result[col].date));
                        var hours = $rootScope.formatIntegerToTwoDigits(parseInt(parseFloat(result[col].time) / 3600));
                        var minutes = $rootScope.formatIntegerToTwoDigits(parseInt((parseFloat(result[col].time) - hours * 3600) / 60));
                        var seconds = $rootScope.formatIntegerToTwoDigits(parseInt(parseFloat(result[col].time) % 60));
                        $rootScope.conquredCols_data.push({
                            col_title: result[col].col_name,
                            col_date: d,
                            col_time: hours + ':' + minutes + ':' + seconds
                        });
                    }
                });
        }
        ...
        $rootScope.loadProfileData = function (homeFactory, $state, $ionicModal) {
        ...
        $rootScope.checkProfileData = function ($ionicModal) {
            $rootScope.newData = {
    ...
    .controller('Email_LoginCtrl', function ($scope, $rootScope, $state, $ionicModal, homeFactory) {
        ...
        $scope.forgot = function () {
            console.log('forgot');
            window.open('http://www.deuxmille.cc/password-reset/');
        }
    ...
    .controller('DeuxmillecolsDetailCtrl', function ($state, $scope, $rootScope, $ionicModal, $ionicSlideBoxDelegate, homeFactory) {
                ...
                for (var i = 0; i < result.length; i++) {
                    if (result[i].ftp && result[i].weight && result[i].bike_weight && result[i].user_col_level) {
                        $rootScope.json_member_result.push({
                            ...
                            member_fname: result[i].firstname
                        });
                    }
                }
        ...
        NativeStorage.getItem('lastViewedMembers', function (val) {
            $rootScope.lastViewedMembers = new Queue(5);
            $rootScope.lastViewedMembers.stac = val.stac;
            console.log($rootScope.lastViewedMembers);
        }, function (err) {
            console.log("Error while fetching from storage.");
            $rootScope.lastViewedMembers = new Queue(5);
        });
        ...
        $scope.choose_member = function (u) {
            ...
            $scope.recalculateForSimul(parseFloat($scope.choose_member_weight), parseFloat($scope.choose_member_ftp), true);
            $rootScope.lastViewedMembers.addItem(u);
            NativeStorage.setItem('lastViewedMembers', $rootScope.lastViewedMembers, function (success) {
