...
.controller('ProfileCtrl', function($scope, $rootScope, $ionicModal, homeFactory) {
  ...
  .then(function (response) {
    ...
    if(result.status == "ok"){
      ...
      $scope.profile_data = {};
      $scope.profile_data.units = result.units != null ? result.units : '';
      $scope.profile_data.ftp = result.ftp != null ? result.ftp : '';
      $scope.profile_data.weight = result.weight != null ? result.weight : '';
      $scope.profile_data.bikeweight = "15";
      $scope.profile_data.pwr = "30";
    }else{
      alert("User Login failed!")
    }
  });
